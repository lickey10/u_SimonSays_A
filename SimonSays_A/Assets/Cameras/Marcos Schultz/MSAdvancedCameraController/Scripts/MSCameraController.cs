...
public class CameraType {
    [Tooltip("A camera or VR Camera Rig must be associated with this variable or a VR Camera Rig associated with VVRCameraRig variable. The camera that is associated here, will receive the settings of this index.")]
    public GameObject CameraOrVRCameraRig;
	public enum TipoRotac{LookAtThePlayer, FirstPerson, FollowPlayer, Orbital, Stop, StraightStop, OrbitalThatFollows, ETS_StyleCamera, FlyCamera, VRCamera}
...
public class SettingsCameraFirstPerson {
    public string CameraName = "First Person";
...
public class SettingsCameraOrbital {
    public string CameraName = "Orbital";
...
public class SettingsCameraOrbitalThatFollows {
    public string CameraName = "Orbital That Follows";
    [Range(1,20)][Tooltip("The speed at which the camera can follow the player.")]
...
public class SettingsCameraETS_StyleCamera {
    public string CameraName = "ETS Style";
    [Range(1,20)][Tooltip("Horizontal camera rotation sensitivity.")]
	public float sensibilityX = 10.0f;
	[Range(1,20)][Tooltip("Vertical camera rotation sensitivity.")]
	public float sensibilityY = 10.0f;
...
public class MSCameraController : MonoBehaviour {
	...
	public bool adjustTheLayers = true;
	...
	void Awake(){
        ...
        for (int x = 0; x < cameras.Length; x++) {
            Camera _camera = cameras[x].CameraOrVRCameraRig.GetComponent<Camera>();

            if (_camera != null) {
                if (cameras[x].Default && cameras[x].Enabled)
                    index = x;

                if (cameras [x].volume == 0) {
					cameras [x].volume = 1;
				}
				initialFieldOfView [x] = _camera.fieldOfView;

				if (cameras [x].rotationType == CameraType.TipoRotac.FirstPerson) {
					_camera.transform.parent = temp.transform;
					originalRotation [x] = _camera.transform.localRotation;
				}
				...
				if (cameras [x].rotationType == CameraType.TipoRotac.FollowPlayer) {
					...
					originalPosition [x].transform.position = _camera.transform.position;
					if (adjustTheLayers) {
				...
				if (cameras [x].rotationType == CameraType.TipoRotac.Orbital) {
					_camera.transform.parent = temp.transform;
					xOrbit [x] = _camera.transform.eulerAngles.y;
					yOrbit [x] = _camera.transform.eulerAngles.x;
					if (adjustTheLayers) {
						targetTransform.gameObject.layer = 2;
						foreach (Transform trans in targetTransform.gameObject.GetComponentsInChildren<Transform>(true)) {
							trans.gameObject.layer = 2;
				...
				distanceFromOrbitalCamera [x] = Vector3.Distance (_camera.transform.position, targetTransform.position);
				...
				if (cameras [x].rotationType == CameraType.TipoRotac.StraightStop) {
					...
					objPosicStopCameras [x].transform.parent = _camera.transform;
				...
				if (cameras [x].rotationType == CameraType.TipoRotac.OrbitalThatFollows) {
					_camera.transform.parent = temp.transform;
					xOrbit [x] = _camera.transform.eulerAngles.x;
					yOrbit [x] = _camera.transform.eulerAngles.y;
				...
				if (cameras [x].rotationType == CameraType.TipoRotac.ETS_StyleCamera) {
					_camera.transform.parent = temp.transform;
					originalRotation [x] = _camera.transform.localRotation;
					originalPositionETS [x] = _camera.transform.localPosition;
				}
				...
				AudioListener audListner = _camera.GetComponent<AudioListener> ();
				if (audListner == null) {
					_camera.transform.gameObject.AddComponent (typeof(AudioListener));
				}
	...
	void EnableCameras (int index){
		if (cameras.Length > 0) {
			for (int x = 0; x < cameras.Length; x++) {
				if (cameras [x].CameraOrVRCameraRig) {
                    Camera _camera = cameras[x].CameraOrVRCameraRig.GetComponent<Camera>();

                    if (x == index && cameras[x].Enabled) {

                        if (_camera)
                            _camera.gameObject.SetActive(true);
                        else
                        {
                            cameras[x].CameraOrVRCameraRig.gameObject.SetActive(true);
                        }

                        switch (cameras[x].rotationType)
                        {
                            case CameraType.TipoRotac.FirstPerson:
                                rotacX = 0.0f;
                                rotacY = 0.0f;

                                currentCameraName = CameraSettings.firstPerson.CameraName;

                                break;
                            case CameraType.TipoRotac.OrbitalThatFollows:
                                tempoOrbit = 0.0f;

                                currentCameraName = CameraSettings.OrbitalThatFollows.CameraName;
                            ...
                            case CameraType.TipoRotac.ETS_StyleCamera:
                                ...
                                currentCameraName = CameraSettings.ETS_StyleCamera.CameraName;

                                break;
                            case CameraType.TipoRotac.FlyCamera:
                                _camera.transform.position = cameras [lastIndex].CameraOrVRCameraRig.transform.position;
							    cameraRotationFly = new Vector2(cameras [lastIndex].CameraOrVRCameraRig.transform.eulerAngles.y, 0);

                                currentCameraName = CameraSettings.FlyCamera.CameraName;

                                break;
                            case CameraType.TipoRotac.FollowPlayer:
                                currentCameraName = CameraSettings.followPlayer.CameraName;

                                break;
                            case CameraType.TipoRotac.LookAtThePlayer:
                                currentCameraName = "Look At The Player";

                                break;
                            case CameraType.TipoRotac.Orbital:
                                currentCameraName = CameraSettings.orbital.CameraName;

                                break;
                            case CameraType.TipoRotac.Stop:
                                currentCameraName = "Stop";

                                break;
                            case CameraType.TipoRotac.StraightStop:
                                currentCameraName = "Straight Stop";

                                break;
                            case CameraType.TipoRotac.VRCamera:
                                currentCameraName = "VR Camera";
                        ...
                        if (_camera)
                            ...
                            cameras[x].CameraOrVRCameraRig.gameObject.SetActive(false);
                    }
				}
			}
		}
	...
	void ManageCameras(){
		for (int x = 0; x < cameras.Length; x++) {
			if (cameras [x].rotationType == CameraType.TipoRotac.FollowPlayer) {
                ...
                if (_camera.isActiveAndEnabled) {
					...
					_camera.transform.parent = playerCamsObj.transform;
				}
			}
		}
		...
		case CameraType.TipoRotac.Stop:
			...
			break;
		case CameraType.TipoRotac.StraightStop:
			tempRotation = Quaternion.LookRotation(objPosicStopCameras[index].transform.position - cameras [index].CameraOrVRCameraRig.transform.position, Vector3.up);
			cameras [index].CameraOrVRCameraRig.transform.rotation = Quaternion.Slerp(cameras [index].CameraOrVRCameraRig.transform.rotation, tempRotation, Time.deltaTime * 15.0f);
			break;
		case CameraType.TipoRotac.LookAtThePlayer:
			cameras [index].CameraOrVRCameraRig.transform.LookAt (targetTransform.position);
			break;
		case CameraType.TipoRotac.FirstPerson:
			...
			cameras [index].CameraOrVRCameraRig.transform.localRotation = Quaternion.Lerp (cameras [index].CameraOrVRCameraRig.transform.localRotation, tempRotation, Time.deltaTime * 10.0f * timeScaleSpeed);
			//fieldOfView
			cameras [index].CameraOrVRCameraRig.GetComponent<Camera>().fieldOfView -= Input.GetAxis (CameraSettings.inputMouseScrollWheel)*CameraSettings.firstPerson.speedScroolZoom*50.0f;
			if (cameras [index].CameraOrVRCameraRig.GetComponent<Camera>().fieldOfView < (initialFieldOfView [index] - CameraSettings.firstPerson.maxScroolZoom)) {
				cameras [index].CameraOrVRCameraRig.GetComponent<Camera>().fieldOfView = (initialFieldOfView [index] - CameraSettings.firstPerson.maxScroolZoom);
			}
			if (cameras [index].CameraOrVRCameraRig.GetComponent<Camera>().fieldOfView > initialFieldOfView [index]) {
				cameras [index].CameraOrVRCameraRig.GetComponent<Camera>().fieldOfView = (initialFieldOfView [index]);
			}
			break;
		case CameraType.TipoRotac.FollowPlayer:
			if (!Physics.Linecast (targetTransform.position, originalPosition [index].transform.position)) {
				cameras [index].CameraOrVRCameraRig.transform.position = Vector3.Lerp (cameras [index].CameraOrVRCameraRig.transform.position, originalPosition [index].transform.position, Time.deltaTime * CameraSettings.followPlayer.displacementSpeed);
				...
				if (CameraSettings.followPlayer.ignoreCollision) {
					...
					cameras [index].CameraOrVRCameraRig.transform.position = Vector3.Lerp (cameras [index].CameraOrVRCameraRig.transform.position, tempHit.point, Time.deltaTime * CameraSettings.followPlayer.displacementSpeed);
				}
			}
			//
			if (CameraSettings.followPlayer.customLookAt) {
				tempRotation = Quaternion.LookRotation (targetTransform.position - cameras [index].CameraOrVRCameraRig.transform.position, Vector3.up);
				cameras [index].CameraOrVRCameraRig.transform.rotation = Quaternion.Slerp (cameras [index].CameraOrVRCameraRig.transform.rotation, tempRotation, Time.deltaTime * CameraSettings.followPlayer.spinSpeedCustomLookAt);
			} else {
				cameras [index].CameraOrVRCameraRig.transform.LookAt (targetTransform.position);
			}
			break;
		case CameraType.TipoRotac.Orbital:
			...
			if (Physics.Linecast (targetTransform.position, cameras [index].CameraOrVRCameraRig.transform.position, out tempHit)) {
			...
			newPosition = cameras [index].CameraOrVRCameraRig.transform.position;
			tempRotation = cameras [index].CameraOrVRCameraRig.transform.rotation;
			cameras [index].CameraOrVRCameraRig.transform.rotation = Quaternion.Lerp(tempRotation,tempRotation2,Time.deltaTime*5.0f*timeScaleSpeed);
			cameras [index].CameraOrVRCameraRig.transform.position = Vector3.Lerp(newPosition,otherPosition,Time.deltaTime*5.0f*timeScaleSpeed);
			break;
		case CameraType.TipoRotac.OrbitalThatFollows:
			...
			if(orbitalAtiv == true){
				...
				if (!Physics.Linecast (targetTransform.position, originalPosition [index].transform.position)) {
					cameras [index].CameraOrVRCameraRig.transform.position = Vector3.Lerp (cameras [index].CameraOrVRCameraRig.transform.position, originalPosition [index].transform.position, Time.deltaTime * CameraSettings.OrbitalThatFollows.displacementSpeed);
				}
					...
					if (CameraSettings.OrbitalThatFollows.ignoreCollision) {
						...
						cameras [index].CameraOrVRCameraRig.transform.position = Vector3.Lerp(cameras [index].CameraOrVRCameraRig.transform.position, tempHit.point,Time.deltaTime * CameraSettings.OrbitalThatFollows.displacementSpeed);
					}
				}
				//
				if (CameraSettings.OrbitalThatFollows.customLookAt) {
					tempRotation = Quaternion.LookRotation (targetTransform.position - cameras [index].CameraOrVRCameraRig.transform.position, Vector3.up);
					cameras [index].CameraOrVRCameraRig.transform.rotation = Quaternion.Slerp (cameras [index].CameraOrVRCameraRig.transform.rotation, tempRotation, Time.deltaTime * CameraSettings.OrbitalThatFollows.spinSpeedCustomLookAt);
				} else {
					cameras [index].CameraOrVRCameraRig.transform.LookAt (targetTransform.position);
				}
			}
		...
		case CameraType.TipoRotac.ETS_StyleCamera:
			...
			cameras [index].CameraOrVRCameraRig.transform.localPosition = Vector3.Lerp (cameras [index].CameraOrVRCameraRig.transform.localPosition, newPosition, Time.deltaTime * 10.0f);
			...
			cameras [index].CameraOrVRCameraRig.transform.localRotation = Quaternion.Lerp (cameras [index].CameraOrVRCameraRig.transform.localRotation, tempRotation, Time.deltaTime * 10.0f * timeScaleSpeed);
			//fieldOfView
			cameras [index].CameraOrVRCameraRig.GetComponent<Camera>().fieldOfView -= Input.GetAxis (CameraSettings.inputMouseScrollWheel)*CameraSettings.ETS_StyleCamera.speedScroolZoom*50.0f;
			if (cameras [index].CameraOrVRCameraRig.GetComponent<Camera>().fieldOfView < (initialFieldOfView [index] - CameraSettings.ETS_StyleCamera.maxScroolZoom)) {
				cameras [index].CameraOrVRCameraRig.GetComponent<Camera>().fieldOfView = (initialFieldOfView [index] - CameraSettings.ETS_StyleCamera.maxScroolZoom);
			}
			if (cameras [index].CameraOrVRCameraRig.GetComponent<Camera>().fieldOfView > initialFieldOfView [index]) {
				cameras [index].CameraOrVRCameraRig.GetComponent<Camera>().fieldOfView = (initialFieldOfView [index]);
			}
		...
		case CameraType.TipoRotac.FlyCamera:
			...
			cameras [index].CameraOrVRCameraRig.transform.rotation = Quaternion.AngleAxis (cameraRotationFly.x, Vector3.up);
			cameras [index].CameraOrVRCameraRig.transform.rotation *= Quaternion.AngleAxis (cameraRotationFly.y, Vector3.left);
			//
			speedCamFly = CameraSettings.FlyCamera.movementSpeed;
			if (Input.GetKey (CameraSettings.FlyCamera.speedKeyCode)) {
				speedCamFly *= 3.0f;
			}
			cameras [index].CameraOrVRCameraRig.transform.position += cameras [index].CameraOrVRCameraRig.transform.right * speedCamFly * Input.GetAxis(CameraSettings.FlyCamera.horizontalMove) * Time.deltaTime;
			cameras [index].CameraOrVRCameraRig.transform.position += cameras [index].CameraOrVRCameraRig.transform.forward * speedCamFly * Input.GetAxis(CameraSettings.FlyCamera.verticalMove) * Time.deltaTime;
			//
			if(Input.GetKey(CameraSettings.FlyCamera.moveUp)){
				cameras [index].CameraOrVRCameraRig.transform.position += Vector3.up * speedCamFly * Time.deltaTime;
			}
	...
	void LateUpdate(){
		if (cameras.Length > 0 && Time.timeScale > 0) {
			if (cameras [index].CameraOrVRCameraRig) {
				ManageCameras ();
			}
		}
	}
    ...
    void OnGUI()
    {
        if (displayMessage)
        {
            GUI.Label(new Rect(Screen.width / 2, 5, 130f, 25f), currentCameraName, "box");
        }
    }
}